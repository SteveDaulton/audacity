  ...
  (do ((i 0 (+ i 1))  ; i is index into string
      ...
      ((or (not (< i len)) err)
       (if (or (> ltr 0) ; must be at least one letter, or
           ...
           nil))
    (setq chr (char str i))
    (cond ((>= (char-code chr) 128) ;; if multi-byte code is encountered,
           ;; all bets are off -- assume this is a valid symbol
           (setf ltr 1) ;; to accept as symbol, ltr must be > 0
           (setf i (1- (length str)))) ;; pretend we are at the end of loop
